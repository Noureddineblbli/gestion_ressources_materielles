...
import { Clock, Printer, Laptop, X } from "lucide-react"
...
const initialRequests = [
  ...
  },
  {
    ...
    assignment: {
      date: "24 mars 2025",
      resource: "Ordinateur portable Dell Latitude",
      mark: "Dell",
      price: "899,99 €",
    },
  },
...
export default function TeacherRequests() {
  ...
  const handleViewAssignment = (request: any) => {
    if (request.status === "assigned" && request.assignment) {
      setSelectedRequest(request)
  ...
  const handleStatusClick = (request: any) => {
    if (request.status === "modified") {
      handleViewHistory(request)
    } else if (request.status === "assigned") {
      handleViewAssignment(request)
    }
  }

  return (
    <TeacherLayout title="Suivre demande" subtitle="Suivez l'état de vos demandes de ressources">
      <Tabs defaultValue="all">
        ...
        <TabsContent value="all" className="mt-0">
          <RequestsList requests={requests} onStatusClick={handleStatusClick} />
        </TabsContent>
          ...
          <div className="border rounded-md p-4 bg-blue-50">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="font-medium">{selectedRequest?.assignment?.mark}</p>
              </div>
              <div>
...
function RequestsList({ requests, onStatusClick }: RequestsListProps) {
            ...
            <tr key={request.id} className="border-t hover:bg-muted/20">
              ...
              <td className="px-4 py-3">
                <div
                  ...
                  onClick={() => onStatusClick(request)}
                >
                  {request.status === "modified" && <Clock className="h-3 w-3 mr-1" />}
                  {request.status === "assigned" && <Laptop className="h-3 w-3 mr-1" />}
                  {request.status === "modified"
                    ? `Validé après modification le ${request.history.date}`
                    : request.statusText}
                </div>
              </td>
            </tr>
